...
Fst between all populations using adegenet: 
```
...
heatmap.2(as.matrix(CHall.fst), trace="none", dendrogram="row", RowSideColors=Elev.colour, col=shadesOfGrey)  ##RowSideColors is for the dendrogram. Colour order should be the same as the input. Here I coloured by elevation

heatmap.2(as.matrix(CHall.fst), trace="none", dendrogram="row", RowSideColors=DendColours, col=shadesOfGrey)  ##Coloured by geography
