...
library(MASS)
dens <- kde2d(Dgeo.CHall,Dgen.Nei, n=300)
myPal <- colorRampPalette(c("white","blue","gold", "orange", "red"))
plot(Dgeo.CHall, Dgen.Nei, pch=20,cex=.5, xlab="Geographic distance (km)", ylab="Nei's genetic distance")
image(dens, col=transp(myPal(300),.7), add=TRUE)
abline(fit <- lm(Dgen.Nei~Dgeo.CHall))
legend("bottomright", bty="n", legend=paste("R2 =", format(summary(fit)$adj.r.squared, digits=4)))  ##and paste R2
...
##CHN
CHN.coords <- subset(CHall.coords, region.L2==c("CHN.BE", "CHN.VD", "CHN.Mid"), select=site:region)
...
ibd.CHS <- mantel.randtest(Dgen.Euc.CHS, Dgeo.CH)
...
myPal <- colorRampPalette(c("white", "blue", "gold", "orange", "red"))
plot(Dgeo.CHS, Dgen.Euc.CHS, pch=20, cex=.5, xlab="Geographic distance (km)", ylab="Genetic Euclidean Dist")
image(dens, col=transp(myPal(300),.7), add=TRUE)
...
##CHS.VS
CHS.coords <- subset(CHall.coords, region==c("CHS.VS", "CHS.TI"), select=site:region)
CHS_lon.lat <- cbind(CHS.coords$long, CHS.coords$lat)
distance.matrix.CHS <- rdist.earth(CHall_lon.lat)  ##great circle dist based on the coordinates
Dgeo.CHS <- as.dist(distance.matrix.CHS)
...
L2.names.factor <- as.factor(region.names$region.CHS.CHN)
CH.L2 <- CHall
CH.L2$pop <- L2.names.factor
CH.L2 <- seppop(CH.L2)
L2.indivpopnames <- as.factor(region.names$pop.L2.perindiv)
CH.L2$CHN$pop <- L2.indivpopnames
CHN.genpop <- genind2genpop(CH.L2$CHN)
Dgen.Euc.CHN <- dist.genpop(CHN.genpop, method=2)
...
L3.names.factor <- as.factor(region.names$region.L3)
CH.L3 <- CHall
CH.L3$pop <- L3.names.factor
CH.L3 <- seppop(CH.L3)
CHS.VS.genpop <- genind2genpop(CH.L3$CHS.VS)
Dgen.Euc.CHS.VS <- dist.genpop(CHS.VS.genpop, method=2)

CHS.TI.genpop <- genind2genpop(CH.L3$CHS.TI)
