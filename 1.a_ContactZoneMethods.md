...
I eventually manipulated this in excel. 

1. rename all the loci.

2. Start all names with "Allelexxx_A" (check example "freq3_MajorAlleleFreq.csv" & "..txt")

3. Copy major and minor allele frequency into one file and sort. Final input "InputEAST.txt"
...
setwd("2016RADAnalysis/1.2_Phylo/HZAR/")
freq <- read.table("plink.frq.strat", header = T, stringsAsFactors = F)
head(freq)
str(freq)
...
#freq$A1 <- with(freq, 1-MAF)##add column with major allele freq
#head(freq)
...
##calculate major allele freq in excel and save this second datafile with the same headings. 
##Read this second dataframe and interleave with freq3

df3.Major <- read.table("freq3_MajorAlleleFreq.txt", header = T)
head(df3.Major)
summary(df3.Major)

df3.maf <- read.csv("freq3.csv", header=T)
head(df3.maf)
summary(df3.maf)
