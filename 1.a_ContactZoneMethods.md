...
```
/usr/local/bgc-1.03/estpost -i mcmcout.hdf5 -p alpha -o a6.3a.out -s 0 -c 0.95 -w 0

/usr/local/bgc-1.03/estpost -i mcmcout.hdf5 -p beta -o b6.3a.out -s 0 -c 0.95 -w 0

/usr/local/bgc-1.03/estpost -i mcmcout.hdf5 -p LnL -o LnL6.3a.out -s 2 -c 0.95 -w 0

/usr/local/bgc-1.03/estpost -i mcmcout.hdf5 -p hi -o hi6.3a.out -s 0 -c 0.95 -w 0
```
...
```
for f in $(ls snp_batch*); do ./bayenv2 -i $f -e EAST.poporder.env -m EASTmatrix.out -k 100000 -r $RANDOM -p 28 -n 1 -t; done
```
