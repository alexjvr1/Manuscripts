...
Filter for a mean depth of 3. (This should already be filtered for in the pyrad)
```
nothing changes
```
...
```
vcftools --vcf FQs2.recode.vcf --missing-indv
...
vcftools --vcf FQs2.recode.vcf --remove lowDP.indiv --recode --recode-INFO-all --out FQ.imiss50
...
And filter loci with >10% missing data
```
vcftools --vcf FQ.imiss50.recode.vcf --max-missing 0.10 --maf 0.05 --recode --recode-INFO-all --out FQfinal


After filtering, kept 168 out of 168 Individuals
Outputting VCF file...
After filtering, kept 24489 out of a possible 39540 Sites
```
...
![alt_txt][Fig5]
[Fig5]:https://cloud.githubusercontent.com/assets/12142475/11095952/db4243c4-8897-11e5-848d-d9e7d71812db.png
