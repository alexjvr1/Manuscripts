...
I downsampled the number of initial reads in pyRAD (see below), because Finland kept dropping out due to low coverage. 

Once the genome is ready, I can map and call more SNPs. Ill do this later, or for a comparison between the Swiss and Swedish datasets. 

i.e. there is a second pyRAD run using all of the available reads. 
...
I calculated the average heterozygosity by counting the het occurrence across the entire sequence in the raw .phy output from the subset pyRAD run: 

gdcsrv1.ethz.ch:/gdc_home4/alexjvr/SEFinalSamples/subset1Mil.SE/outfiles

/Users/alexjvr/2016RADAnalysis/5_SE.MS1/Analyses_old/summstats/SEsubsetAvgHet

Counting: 

```
#Missing data
tr -d -c 'N\n'< SEsubset.phy |awk '{print length; }'
##gaps
sed 's/[^-]//g' SEsubset.phy |awk '{print length}'
...
grep -Eo '^[^ ]+' SEsubset.phy 
```


Import into R and draw the graph
```
sort.subset.avgHet$pop <- factor(sort.subset.avgHet$pop, levels=sort.subset.avgHet$pop)   ##sort pop.nr. Numbers from south to North

qplot(pop, avghet, data=sort.subset.avgHet, geom=c("boxplot", "jitter"))

```


![AvgHet](https://cloud.githubusercontent.com/assets/12142475/20067374/2d64a30a-a515-11e6-89d1-4c4afa428249.png)


And for only the 167 individuals in the final dataset (see below)

```
SE167 <- subset.avgHet[-c(18, 19, 100, 101, 102, 103, 104, 156, 157, 158)]  ##remove the individual lines in R

sort.SE167 <- SE167[order(SE167$nr),]  ##sort South to north

sort.SE167$pop <- factor(sort.SE167$pop, levels=sort.SE167$pop) ##pop is a factor so it doesnt get resorted. 
```


![AvgHet167](https://cloud.githubusercontent.com/assets/12142475/20068430/fd57297c-a518-11e6-8733-3997fefed080.png)
...
Calculate LD within each population (with n>5): 

First subset the Plink file into the different populations: 

```
plink --file SEsubset.Final --keep DE_B --recode --recodeA --out DE_B.SEFinal  ##some of the outfiles are named $.plink
										## DE_B name file is two columns, one with indiv name and one with pop. In most of my plink files no pop info is available, so the indiv name is doubled. 
```

LD
```
plink --file DE_B.SEFinal --r2 --out DE_B.ld
```

Upload this in R and graph: 
```
DE_W.ld <- read.table("DE_W.ld.ld", header=T)
DE_K.ld <- read.table("DE_K.ld.ld", header=T)
FIN.ld <- read.table("FIN.ld.ld", header=T)
Kir_G.ld <- read.table("Kir_G.ld.ld", header=T)
Kir_L.ld <- read.table("Kir_L.ld.ld", header=T)
LT1.ld <- read.table("LT1.ld.ld", header=T)
LT3.ld <- read.table("LT3.ld.ld", header=T)
SK_Ho.ld <- read.table("SK_Ho.ld.ld", header=T)
SK_SF.ld <- read.table("Sk_SF.ld.ld", header=T)
SK_SL.ld <- read.table("Sk_SL.ld.ld", header=T)
UP_Gra.ld <- read.table("UP_Gra.ld.ld", header=T)
UP_K.ld <- read.table("UP_K.ld.ld", header=T)
UP_O.ld <- read.table("UP_O.ld.ld", header=T)
Um_Gr.ld <- read.table("Um_Gr.ld.ld", header=T)
Um_Taf.ld <- read.table("Um_Taf.ld.ld", header=T)


par(mfrow=c(3,3))
hist(DE_B.ld$R2, main="LD DE_B", xlab=NULL)
hist(DE_W.ld$R2, main="LD DE_W", xlab=NULL)
hist(DE_K.ld$R2, main="LD DE_K", xlab=NULL)
hist(SK_Ho.ld$R2, main="LD SK_Ho", xlab=NULL)
hist(SK_SF.ld$R2, main="LD SK_SF", xlab=NULL)
hist(SK_SL.ld$R2, main="LD SK_SL", xlab=NULL)
hist(UP_Gra.ld$R2, main="LD UP_Gra", xlab=NULL)
hist(UP_K.ld$R2, main="LD UP_K", xlab=NULL)
hist(UP_O.ld$R2, main="LD UP_O", xlab=NULL)


par(mfrow=c(2,2))
hist(Um_Gr.ld$R2, main="LD Um_Gr", xlab=NULL)
hist(Um_Taf.ld$R2, main="LD Um_Taf", xlab=NULL)
hist(LT1.ld$R2, main="LD LT1", xlab=NULL)
hist(LT3.ld$R2, main="LD LT3", xlab=NULL)

par(mfrow=c(2,2))
hist(Kir_G.ld$R2, main="LD Kir_G", xlab=NULL)
hist(Kir_L.ld$R2, main="LD Kir_L", xlab=NULL)
hist(FIN.ld$R2, main="LD FIN", xlab=NULL)
```

![LD1](https://cloud.githubusercontent.com/assets/12142475/20096386/eb766b6e-a5a9-11e6-804e-02b7e6cf9052.png)

![LD2](https://cloud.githubusercontent.com/assets/12142475/20096395/f647d578-a5a9-11e6-91af-86ce51923ff0.png)

![LD3](https://cloud.githubusercontent.com/assets/12142475/20096404/006aac38-a5aa-11e6-99a0-6df2a50fb822.png)
...
In Adegenet. Convert vcf to structure format. 
...
SE.Final <- read.structure("SEsubset.167.5189.str")

 How many genotypes are there? 167

 How many markers are there? 5189
...
##use pop names i.s.o numbers from .str input  (17 populations)
popnames.SE.Final <- read.table("pop.names", header=F)
popnames.SE.Final.factor <- as.factor(popnames.SE.Final$V3)
SE.Final@pop <- popnames.SE.Final.factor 
```


Some summary stats with adegenet
```
sum.SE.Final <- summary(SE.Final)
...
and hierfstat
```
hier.SE.Final <- genind2hierfstat(SE.Final)
hier.SE.Final.stats <- basic.stats(hier.SE.Final, diploid=T, digits=2)

names(hier.SE.Final.stats)
...
```
pops17 <- seppop(SE.Final)  ##adegenet function

stats.DE.B <- summary(pop17$DE_B)  ##adegenet stats

hier.DE.B <- genind2hierfstat(pops17$DE_B)  ##hierfstat
```
...
```
par(mfrow=c(3,2))
hist(stat.DEB$Hobs, main="DEB Hobs")
hist(stat.DEB$Hexp, main="DEB Hexp")
hist(stat.DEW$Hobs, main="DEW Hobs")
hist(stat.DEW$Hexp, main="DEW Hexp")
hist(stat.DEK$Hobs, main="DEK Hobs")
hist(stat.DEK$Hexp, main="DEK Hexp")
...
I havent figured out how to write a script to loop through fastStructure, but change the output file for each run. I.e. I have to manually run K 1-10 x 10 runs. According to the google group, this is how it has to be done. The runs take just a few seconds each.
...
Reduce the number of variables using Spearmans Rho
```
...
**Im doing this in R on the gdc server, because my Rstudio keeps crashing. I think the problem was that I hadnt corrected the BIOclim file to include data for all individuals. 
...
export.lfmmProject("filename")  ##This creates a zip of the lfmm project, which can then easily be moved between computers. 
```

Looking specifically at the loci associated with season length: 

```
SEproj <- import.lfmmProject("SEsubset.4env_SE_lfmmProject.zip")

zs = z.scores(SEproj, K=2, d=1)
zs.stouffer = apply(zs, MARGIN=1, median)
lambda =median(zs^2)/0.456
lambda
[1] 4.221895
cp.values =pchisq(zs.stouffer^2/lambda, df=1, lower=F)

hist(cp.values)
```

![cp.values_Fig1](https://cloud.githubusercontent.com/assets/12142475/20217460/5076ffec-a820-11e6-804f-d48b279ad1d7.png)
...
candidates.temp <- order(cp.values)[w]

candidates.temp
1066 1675 5082  914 1136  415 2799 1855 4322 3178 5027 1100  159 4070  212
1056 4580 4676 3206  483  655 3952  227 1285  173 4224 2442 1019 4927  391
```

But since some of these are non-variable (or variable in <5 pops or just have really low frequencies), I'm including only 11 loci in the Gradient Forest Analysis

```
1019 173 1285 227 3952 4676 5082 914 2799 1100 4070
```
...
Open in excel and rename the loci according to the vcf order. (see SE.MAF.xlsx)
...
According to one of the runs, there are 60 outliers associated with both BIO02 and BIO05. 28 of these were associated with both variables. 
...
Initial run will be using only 23 of these that are variable in >4 populations. I found that 37 of the loci had 0 frequency in all but 1-2 populations, where they occurred only at low frequencies. I've removed these loci for now. 

Test dataset: SESNP.env.data4GF_20161109.csv 
```
Candidate_X173	Candidate_X3952
Candidate_X391	Candidate_X4070
Candidate_X483	Candidate_X4132
Candidate_X1019	Candidate_X4322
Candidate_X1062	Candidate_X4478
Candidate_X1100	Candidate_X4553
Candidate_X1285	Candidate_X4580
Candidate_X1372	Candidate_X4636
Candidate_X2799	Candidate_X4676
Candidate_X3171	Candidate_X4770
Candidate_X3340	Candidate_X5082
Candidate_X3838	
...
```
#############################
##Random Forest - as used in the wolf paper Schweizer et al. 2016
##Identify ecotypes
##measures how well individuals can be assigned to a group based only on env variables
#############################

setwd("/Users/alexjvr/2016RADAnalysis/5_SE.MS1/Analyses_old/GradForest")
#install.packages("/Users/alexjvr/Downloads/extendedForest_1.6.1.tar.gz") ##need extendedForest first
...
# read in data file with minor allele freqs & env/space variables
gfData <- read.csv("SESNP.env.data4GF_20161109.csv", header=T)  
# get climate & MEM variables
envGF <- gfData[,6:12]

#build the individual SNP datasets
SNP_ref <- gfData[,grep("Reference", colnames(gfData))] #reference
SNP_23candidates <- gfData[,grep("Candidate", colnames(gfData))] #23 Candidate loci
...
# Fit gf models for 23 Candidate SNPs
gf23Candidates <- gradientForest(cbind(envGF, SNP_23candidates), predictor.vars=colnames(envGF),
                        response.vars=colnames(SNP_23candidates), ntree=500, 
                        maxLevel=maxLevel, trace=T, corr.threshold=0.50)

# plot output, see ?plot.gradientForest
type = "O"
plot(gfRef, plot.type=type)
plot(SNP_23candidates, plot.type=type)
################################################################################
```


Gradient Forest for SEtemp dataset (i.e. outliers based on season length: 
```
library(gradientForest)
library(raster)
gf.SEtemp <- read.csv("SESNP.env.temp_20161111.csv", header=T)
envGF <- gf.SEtemp[,6:12]
SNP_ref.SEtemp <- gf.SEtemp[,grep("Reference", colnames(gf.SEtemp))]
temp.SNPs <- gf.SEtemp[,grep("Candidate", colnames(gf.SEtemp))]

maxLevel <- log2(0.368*nrow(envGF)/2)
gf.Ref.model.SEtemp <- gradientForest(cbind(envGF, SNP_ref.SEtemp), predictor.vars=colnames(envGF), response.vars=colnames(SNP_ref.SEtemp), ntree=500, maxLevel=maxLevel, trace=T, corr.threshold=0.5)
Calculating forests for 4722 species
.....................
