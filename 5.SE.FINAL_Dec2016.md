...
outliers.pcadapt <- as.character(outliers)
...
write.table(outliers.pcadapt.names$outliers.pcadapt.names, "pcadapt.outliers", col.names=F, row.names=F, quote=F)  ##write the table #write to file
...
/Users/alexjvr/2016RADAnalysis/5_SE.MS1/DEC2016_SEonly/BayEnv2
...
![Venn.BayEnv](https://cloud.githubusercontent.com/assets/12142475/23129948/da3e3598-f784-11e6-99d7-0f12d4a44385.png)
...
plot the histogram of Fst for the outliers identified with 1)pcadapt, and 2) LFMM, on the full Fst plot to look determine whether the same loci are outliers. 

Run all outlier analyses (pcadapt & LFMM) with 2 datasets: 1) full dataset, 2) reduced dataset including only loci variable in >5 populations. 
...
pcadapt.list <- as.character(pcadapt.outliers$loci)
pcadapt.fst <- stats.SE132.perlocus[pcadapt.outliers,]
pcadapt.fst <- pcadapt.fst$Fst
...
pcadapt.hist <- hist(pcadapt.fst, breaks=58)
...
#plot(RDA1.hist, col=rgb(0,0,1,1/4), xlim=c(-0.15,1.0), add=T)  ##I decided to remove the RDA loci, because they aren't true outlier loci
#plot(RDA2.hist, col=rgb(1,0,0,1/4), xlim=c(-0.15,1.0), add=T)  ##instead I'm using pcadapt
plot(pcadapt.hist, col=rgb(1,0,0,1/4), xlim=c(-0.15,1.0), add=T)
...
pdf(file="Fst.outlierplot.XtX.pcadapt_20170531.pdf")
plot(fstall.hist, xlim=c(-0.15,1.0))
plot(pcadapt.hist, col=rgb(1,0,0,1/4), xlim=c(-0.15,1.0), add=T)
plot(XtX.hist, col=rgb(1,1,0,1/4), xlim=c(-0.15, 1.0), add=T)
dev.off()
...
![Fst.outliers.oldplot]
(https://cloud.githubusercontent.com/assets/12142475/20971473/d786eb8e-bc91-11e6-897e-40c15fade58b.png)

![Fst.outliers](https://cloud.githubusercontent.com/assets/12142475/26642287/24335e6e-4625-11e7-9ef9-42550903e93c.png)



Fstoutlier figure: Outliers vs EAA

/Users/alexjvr/2016RADAnalysis/5_SE.MS1/DEC2016_SEonly/SumStats
```
Outliers.fst.forplot <- stats.SE132.perlocus[Outlier.names.unique, ]  ##subset the data with only the unique loci
EAA.fst.forplot <- stats.SE132.perlocus[EAA.outliers, ]

Outliers.fst.forplot <- Outliers.fst.forplot$Fst  ##keep only the Fst column
EAA.fst.forplot <- EAA.fst.forplot$Fst

Outlier.hist <- hist(Outliers.fst.forplot, breaks=58)
EAA.hist <- hist(EAA.fst.forplot, breaks=115)  #generate histogram data to overlay plots. 

pdf(file="Fstplots_3datasets_20170220.pdf")
plot(fstall.hist, xlim=c(-0.15,1.0))
plot(BayEnv.hist, col=rgb(0,0,1,1/4), xlim=c(-0.15, 1.0), add=T)
plot(LFMM.hist, col=rgb(1,0,0,1/4), xlim=c(-0.15, 1.0), add=T)
dev.off()

```
This is an old plot using the RDA data i.s.o pcadapt. 

![Fst.plot.all](https://cloud.githubusercontent.com/assets/12142475/23134136/f94e4f54-f793-11e6-88bc-15341de217b8.png)
...
#EAA dataset (LFMM vs BayEnv)

pdf(file="VENN.LFMM.BayENV.pdf")
...
#Outlier dataset (pcadapt vs XtX)

pdf(file="VENN.XtXvsPcadapt.pdf")
d1 <- length(XtX.outliers)
d2 <- length(pcadapt.outliers)

d12 <- length(Reduce(intersect, list(XtX.outliers, pcadapt.outliers)))

draw.pairwise.venn(area1=d1, area2=d2, cross.area=d12,  
category=c("XtX", "pcadapt"),
lty="blank", 
fill=c("yellow", "blue")
)
dev.off()
...
##outlier (pcadapt + XtX) vs EAA

Outlier.names.all <- c(XtX.outliers, pcadapt.outliers)  ##join all names together
Outlier.names.unique <- unique(Outlier.names.all)  ##keep only the unique names

EAA.names.all <- c(LFMM.outliers, BayEnv.outliers)
EAA.outliers <- unique(EAA.names.all)

pdf(file="VENN.Outlier.pcadapt.EAA.pdf")
d1 <- length(Outlier.names.unique)
d2 <- length(EAA.outliers)

d12 <- length(Reduce(intersect, list(Outlier.names.unique, EAA.outliers)))

draw.pairwise.venn(area1=d1, area2=d2, cross.area=d12,  
category=c("Outliers", "EAA"),
lty="blank", 
fill=c("yellow", "blue")
)
dev.off()
...
#Outlier dataset (RDA vs XtX)

pdf(file="VENN.XtXvsRDA.pdf")
d1 <- length(XtX.outliers)
d2 <- length(RDA1.list)
d3 <- length(RDA2.list)

d12 <- length(Reduce(intersect, list(XtX.outliers, RDA1.list)))
d13 <- length(Reduce(intersect, list(XtX.outliers, RDA2.list)))
d23 <- length(Reduce(intersect, list(RDA1.list, RDA2.list)))

d123 <- length(Reduce(intersect, list(XtX.outliers, RDA1.list, RDA2.list)))

draw.triple.venn(area1=d1, area2=d2, area3=d3, n12=d12, n13=d13, n23=d23, n123=d123,  
category=c("XtX", "RDA1", "RDA2"),
lty="blank", 
fill=c("yellow", "skyblue1", "blue")
)
dev.off()
...
Outlier.names.all <- c(XtX.outliers, RDA1.list, RDA2.list)  ##join all names together
Outlier.names.unique <- unique(Outlier.names.all)  ##keep only the unique names

EAA.names.all <- c(LFMM.outliers, BayEnv.outliers)
EAA.outliers <- unique(EAA.names.all)

pdf(file="VENN.Outlier.EAA.pdf")
d1 <- length(Outlier.names.unique)
d2 <- length(EAA.outliers)
...
![Venn.outliers](https://cloud.githubusercontent.com/assets/12142475/26642604/44626972-4626-11e7-9977-b4c67db4c53e.png)


![Venn.EAA](https://cloud.githubusercontent.com/assets/12142475/23131163/5b6e38c6-f789-11e6-8d96-89a1bd46956e.png)


![Venn.outliersvsEAA](https://cloud.githubusercontent.com/assets/12142475/26642605/446309e0-4626-11e7-9808-1ca112098ac1.png)


old plots: 
![Venn.outliers]
(https://cloud.githubusercontent.com/assets/12142475/23131151/507040ae-f789-11e6-9901-001f9dad6323.png)

![Venn.outliersvsEAA]
(https://cloud.githubusercontent.com/assets/12142475/23131170/613b9906-f789-11e6-8d4d-8863d17dc060.png)
...
#Function to compute Morans eigenvectors of a listw object
#This functions compute eigenvectors of a doubly centered spatial weighting matrix. 
#Corresponding eigenvalues are linearly related to Morans index of spatial autocorrelation.
#scores=scores.listw(listw, echo = FALSE, MEM.autocor = c("all","positive", "negative"))
#MEM.autocor: A string indicating if all MEMs must be returned or only those corresponding to positive or negative autocorrelation.
#Only positive correlations:
scores=scores.listw(listw, echo = FALSE, MEM.autocor = "positive")
...
#Function to compute and test Morans I for eigenvectors of spatial weighting matrices. 
#This function tests Morans I for each eigenvector of a spatial weighting matrix
...
######Dataset1 (XtX + PCAdapt)
```
##Dataset1: Fstoutliers (PCAdapt + XtX)
##These seem to overlap a lot. I end up with 122 unique loci in total. 


#read in RDA and XtX outlier names

pcadapt.outlier.names <- read.table("pcadapt.outliers", header=F)
pcadapt.outliers.names$outliers1 <- pcadapt.outliers.names$V1
head(pcadapt.outliers.names)
pcadapt.outliers.names$outliers1 <- gsub("X.", "", pcadapt.outliers.names$outliers1)
head(pcadapt.outliers.names)
pcadapt.outliers.names$outliers1 <- gsub("\\.", ":", pcadapt.outliers.names$outliers1)
pcadapt.outliers.names$outliers1
##write.table(pcadapt.outliers.names$outliers1, "pcadapt.outliernames.vcffriendly", col.names=F, quote=F, row.names=F)
...
Fst.outliers.all.new <- rbind(XtX.outliers, pcadapt.outliers.names)  #join the names together in one column
summary(Fst.outliers.all.new)
Fst.outliers.all.new <- subset(Fst.outliers.all.new,!duplicated(Fst.outliers.all.new$V1))  #remove all duplicates
summary(Fst.outliers.all.new)
write.table(Fst.outliers.all.new$outliers1, "Fst.outliers.vcffriendly.new", col.names=F, row.names=F, quote=F)

Fst.outliers.all.new2 <- Fst.outliers.all.new$outliers1

##linux
vcftools --vcf SE132.FINAL.vcf --snps Fst.outliers.vcffriendly.new --recode --recode-INFO-all --out SE132.Fstoutliers.new
After filtering, kept 122 out of a possible 2081 Sites
```

Calculate MAF for all loci in Plink:

```
vcftools --vcf SE132.Fstoutliers.new.recode.vcf --plink --out SE132.Fstoutliers.new.plink

plink --file SE132.Fstoutliers.new.plink --recode --recodeA --out SE132.Fstoutliers.new.plink
```
...
```
plink --file SE132.Fstoutliers.new.plink --within SE132.122.PlinkCluster --freq --out SE132.Fst.122.plink
```
...
SE.MAF <- read.table("SE132.Fst.122.plink.frq.strat", header=T)
head(SE.MAF)
...
write.csv(SE.MAF3, file="SE132.122.MAF.csv")
```


######Dataset 2 (EAA)
```
##Dataset1: EAA (LFMM + BayEnv2)
##These seem to overlap a lot. I end up with 499 loci in total. 


#read in LFMM and BayEnv2 names

LFMM.outliers.names <- read.table("/Users/alexjvr/2016RADAnalysis/5_SE.MS1/DEC2016_SEonly/LFMM/SE132.FINAL/LFMM.alloutliers",
header=F)
ENV.outlier.names$LFMM <- LFMM.outliers.names$V1
ENV.outlier.names <- LFMM.outliers.names$V1
summary(ENV.outlier.names)
ENV.outlier.names <- as.data.frame(ENV.outlier.names)
summary(ENV.outlier.names)
ENV.outlier.names
colnames(ENV.outlier.names) <- "LFMM"
BayEnv.outlier.names <- read.table("/Users/alexjvr/2016RADAnalysis/5_SE.MS1/DEC2016_SEonly/BayEnv2/BayEnv.alloutliers")
BayEnv.outlier.names <- read.table("/Users/alexjvr/2016RADAnalysis/5_SE.MS1/DEC2016_SEonly/BayEnv2/BayEnv.alloutliers", header=F)
BayEnv.outlier.names
LFMM.outliers.names
BayEnv.outlier.names
ENV.outlier.names <- NULL
ENV.outlier.names <- rbind(LFMM.outliers.names, BayEnv.outlier.names)
summary(ENV.outlier.names)
ENV.outlier.names.unique <- subset(ENV.outlier.names,!duplicated(ENV.outlier.names$V1))

ENV.outlier.names.unique$V1 <- gsub("X.", "", ENV.outlier.names.unique$V1)
ENV.outlier.names.unique$V1 <- gsub("\\.", ":", ENV.outlier.names.unique$V1)
ENV.outlier.names.unique

write.table(ENV.outlier.names.unique$V1, "SE132.ENVoutlier.499.vcffriendly", col.names=F, row.names=F, quote=F)


##linux
vcftools --vcf SE132.FINAL.vcf --snps SE132.ENVoutlier.499.vcffriendly --recode --recode-INFO-all --out SE132.ENVoutliers
After filtering, kept 499 out of a possible 2081 Sites
```

Calculate MAF for all loci in Plink:

```
vcftools --vcf SE132.ENVoutliers.recode.vcf --plink --out SE132.ENVoutliers.plink

plink --file SE132.ENVoutliers.plink --recode --recodeA --out SE132.ENVoutliers.plink
```

Find the sample names in the *nosex file, and add pop names (i.e. 3 columns) to create a file for specifying clusters > SE132.Env.PlinkCluster 

And calculate MAF with Plink

```
plink --file SE132.ENVoutliers.plink --within SE132.Env.PlinkCluster  --freq --out SE132.ENVoutliers.plink
```
...
SE.MAF.Env <- read.table("SE132.ENVoutliers.plink.frq.strat", header=T)
head(SE.MAF.Env)

SE.MAF.Env <- SE.MAF.Env[,c(3,2,6)]

library("ggplot2")
library("reshape2")

SE.MAF2.Env <- melt(SE.MAF.Env, id.vars = c("CLST", "SNP"), variable_name = c("MAF"))
str(SE.MAF2.Env)
head(SE.MAF2.Env)


SE.MAF3.Env <- dcast(SE.MAF2.Env, formula= CLST ~ SNP)
head(SE.MAF3.Env)
colnames(SE.MAF3.Env) <- paste("X", colnames(SE.MAF3.Env), sep=".")  ##Change colnames, so that excel doesn't change the SNP names
write.csv(SE.MAF3.Env, file="SE.MAF3.Env.csv")
```


######Dataset 3 (Neutral:All-(EAA+Fst))
```
##Dataset3: Neutral1 (500)
 
#combine outlier and EAA loci. 


Fst.outliers.all.new2 <- as.data.frame(Fst.outliers.all.new$V1)
colnames(Fst.outliers.all.new2) <- "V1"
all.outliers <- rbind(Fst.outliers.all.new2, ENV.outlier.names)

all.outliers.unique <- subset(all.outliers,!duplicated(all.outliers$V1))

all.outliers.unique$V1 <- gsub("X.", "", all.outliers.unique$V1)
all.outliers.unique$V1 <- gsub("\\.", ":", all.outliers.unique$V1)
all.outliers.unique
summary(all.outliers.unique)


write.table(all.outliers.unique$V1, "Neutralv1.vcffriendly", col.names=F, row.names=F, quote=F)


##linux
vcftools --vcf SE132.FINAL.vcf --exclude Neutralv1.vcffriendly --recode --recode-INFO-all --out SE132.Neutralv1
After filtering, kept 1516 out of a possible 2081 Sites
```

Calculate MAF for all loci in Plink:

```
vcftools --vcf SE132.Neutralv1.recode.vcf --plink --out SE132.Neutralv1.plink

plink --file SE132.Neutralv1.plink --recode --recodeA --out SE132.Neutralv1.plink
```

Find the sample names in the *nosex file, and add pop names (i.e. 3 columns) to create a file for specifying clusters > SE132.Neutralv1.PlinkCluster 

And calculate MAF with Plink

```
plink --file SE132.Neutralv1.plink --within SE132.Neutralv1.PlinkCluster  --freq --out SE132.Neutralv1.plink
```
...
SE.MAF.Nv1 <- read.table("SE132.Neutralv1.plink.frq.strat", header=T)
head(SE.MAF.Nv1)

SE.MAF.Nv1 <- SE.MAF.Nv1[,c(3,2,6)]

library("ggplot2")
library("reshape2")

SE.MAF2.Nv1 <- melt(SE.MAF.Nv1, id.vars = c("CLST", "SNP"), variable_name = c("MAF"))
str(SE.MAF2.Nv1)
head(SE.MAF2.Nv1)


SE.MAF3.Nv1 <- dcast(SE.MAF2.Nv1, formula= CLST ~ SNP)
head(SE.MAF3.Nv1)
colnames(SE.MAF3.Nv1) <- paste("X", colnames(SE.MAF3.Nv1), sep=".")  ##Change colnames, so that excel doesn't change the SNP names
write.csv(SE.MAF3.Nv1, file="SE.MAF3.Nv1.csv")
```


######Dataset 4 (Neutral:GF)
```
...
library(raster)
gf.SEtemp <- read.csv("SE132.235.SNP.env.data5GF_20161209.csv", header=T)  ##import all environmental info
envGF <- gf.SEtemp[,9:17]  #extract env info
#envGF <- gf.SEtemp[,4:12]
#envGF$MEM1 <- as.numeric(envGF$MEM1)
#envGF$MEM2 <- as.numeric(envGF$MEM2)
#envGF$MEM3 <- as.numeric(envGF$MEM3)

##create input files for each MAF dataset
SE.MAF3.Fst <- SE.MAF3
SNP_Fstcandidate.SE132 <- SE.MAF3.Fst[,grep("X.", colnames(SE.MAF3.Fst))]   
#SNP_candidate.SE132 <- gf.SEtemp[,grep("X.", colnames(gf.SEtemp))]

SNP_ENVcandidate.SE132 <- SE.MAF3.Env[,grep("X.", colnames(SE.MAF3.Env))]
SNP_NEUTRAL.SE132 <- SE.MAF3.Nv1[,grep("X.", colnames(SE.MAF3.Nv1))]
