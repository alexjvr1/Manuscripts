...
I'm comparing the full dataset with a reduced dataset (Ho, UT3, and LT2 removed)
```
...
library(adegenet)
library(hierfstat)
SE120.genind <- read.structure("SE120.plink.str")
hier.SE120 <- genind2hierfstat(SE120.genind)
SE120.genind@pop
SE120.pop <- read.table("SE120.pop", header=T)
SE120.pop.factor <- as.factor(SE120.pop$pop)
SE120.genind@pop <- SE120.pop.factor
SE120.genind@pop
hier.SE120 <- genind2hierfstat(SE120.genind)
stats.SE120 <- basic.stats(hier.SE120)
stats.SE120.perlocus <- stats.SE120$perloc

par(mfrow=c(2,1))
hist(stats.SE120.perlocus$Fst, xlim=c(-0.2, 1.0), breaks=120)
hist(stats.SEall.145.perloc$Fst, xlim=c(-0.2, 1.0), breaks=120)
```

![alt_txt][Fst.perloc]
[Fst.perloc]:https://cloud.githubusercontent.com/assets/12142475/20840908/26b89fa6-b8b2-11e6-83c3-101c941e470c.png
...
/Users/alexjvr/2016RADAnalysis/5_SE.MS1/DEC2016_SEonly/LFMM/SE133/
...
```
#
env <- read.csv("SE133.5env.variables", header=F) ###read in the environmental data. One line perindividual. Same order as .vcf file
...
```
library(LEA)
genotype = vcf2lfmm("SE33.FINAL.recode.vcf")

	- number of detected individuals:	133
	- number of detected loci:		2041
...
The project is saved into :
 SE145.FINAL.snmfProject 

To load the project, use:
 project = load.snmfProject("SE133.FINAL.snmfProject")

To remove the project, use:
 remove.snmfProject("SE133.FINAL.snmfProject")
```
...
```
![alt_txt][snmf2041]
[snmf2041]:https://cloud.githubusercontent.com/assets/12142475/20881141/2eedd35a-badb-11e6-807e-3ba4fb5b28a4.png
...
![alt_txt][barplot2041]
[barplot2041]:https://cloud.githubusercontent.com/assets/12142475/20881156/454de6ee-badb-11e6-8b2a-91713986e432.png
