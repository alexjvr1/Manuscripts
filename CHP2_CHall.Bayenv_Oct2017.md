...
grep "ITER = 500$" -A24 CHS275.matrix1.out > CHS.500
grep "ITER = 500$" -A24 CHS275.matrix2.out > CHS.500.2

grep "ITER = 50000" -A24 CHS275.matrix1.out > CHS.50k
grep "ITER = 50000" -A24 CHS275.matrix2.out > CHS.50k.2

grep "ITER = 75000" -A24 CHS275.matrix1.out > CHS.75k
grep "ITER = 75000" -A24 CHS275.matrix2.out > CHS.75k.2

grep "ITER = 100000" -A24 CHS275.matrix1.out > CHS.100k
grep "ITER = 100000" -A24 CHS275.matrix2.out > CHS.100k.2
...
###There were duplicated snp_batch... in CHS.Run3, so I had to print only the unique values: 

awk '!seen[$1]++' bf_environ.ENVIRONFILE.CHS.n5.env > new.bf
```
...
These can be rerun in the same folder and the results will be added to the end of the old XtX and bf outputs. 

copy to Mac and sort using 

```
sort -k 1 bf... > sorted
mv sorted bf.. 

##-k 1 means sort based on the first column
```
...
###CZ
BF.CZ.Run1 <- read.table("CZ.Run1/bf_environ.CZ.n5.p38.ENVIRONFILE", header=F)
...
BF.CZ.Run2 <- read.table("CZ.Run2/bf_environ.CZ.n5.p38.ENVIRONFILE", header=F)
...
BF.CZ.Run3 <- read.table("CZ.Run3/bf_environ.CZ.n5.p38.ENVIRONFILE", header=F)
...
###CHS.TI
BF.CHS.TI.Run1 <- read.table("CHS.TI.Run1/bf_environ.CHS.TI.n5.p14.ENVIRONFILE", header=F)
...
BF.CHS.TI.Run2 <- read.table("CHS.TI.Run2/bf_environ.CHS.TI.n5.p14.ENVIRONFILE", header=F)
...
BF.CHS.TI.Run3 <- read.table("CHS.TI.Run2/bf_environ.CHS.TI.n5.p14.ENVIRONFILE", header=F)
